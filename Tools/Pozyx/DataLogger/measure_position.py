...
class ReadyToLocalize(object):
    ...
    def __init__(self, pozyx, osc_udp_client, anchors, algorithm=POZYX_POS_ALG_UWB_ONLY, dimension=POZYX_3D, height=1000, remote_id=None):
        ...
        # Enable a file reader
        self.file_ = open(os.path.join(sys.path[0], datetime.now().strftime("%c"))+".csv", "a")
        self.sensor_write = csv.writer(self.file_, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL)
    ...
    def setup(self):
        ...
        self.sensor_write.writerow(self.anchors)
        self.sensor_write.writerow(["date_time", "x", "y", "z"])
    ...
    def printPublishPosition(self, position):
        ...
        self.sensor_write.writerow([datetime.now().strftime("%-H:%-M:%-S:%f"), position.x, position.y, position.z])
...
if __name__ == "__main__":
    ...
    anchors = [DeviceCoordinates(0x6E2B, 1, Coordinates(0, 645, 1214)),
               DeviceCoordinates(0x676C, 1, Coordinates(2737, -410, 1913)),
               DeviceCoordinates(0x670A, 1, Coordinates(3651, 4120, 1853)),
               DeviceCoordinates(0x6E22, 1, Coordinates(1541, 4450, 1775))]
